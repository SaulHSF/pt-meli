...
export function ExecutiveSummary({ metrics, loading }: Readonly<ExecutiveSummaryProps>) {
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-20 animate-pulse rounded-lg bg-[var(--color-bg-secondary)]" />
          ))}
        </div>
      </section>
    );
  ...
  return (
    ...
    >
      ...
      <p id="summary-desc" className="mb-4 text-sm text-[var(--color-text-secondary)]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {METRIC_CARDS.map(({ key, label, value, accent }) => (
          <div
            key={key}
            className={`rounded-lg border border-[var(--color-border)] border-l-4 p-3 ${accent} transition-transform duration-150 hover:-translate-y-0.5`}
          >
